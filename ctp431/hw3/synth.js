var Voice = function(context, frequency, amplitude, parameters, effect_node) {
	...
	this.lfoEnv.connect(this.osc.frequency);
	...
	this.lfoEnv.gain.value = parameters.lfoDepth;
...
Voice.prototype.off = function() {
	var param_a = this.ampEnv.gain;
	var param_f = this.filter.frequency;	
	var now = this.context.currentTime;

	param_a.cancelScheduledValues(now);
	param_a.setValueAtTime(param_a.value, now);
	param_a.exponentialRampToValueAtTime(0.001, now + this.ampEnvReleaseTime);

	param_f.cancelScheduledValues(now);
	param_f.setValueAtTime(param_f.value, now);
	param_f.exponentialRampToValueAtTime(0.001, now + this.filterEnvReleaseTime);
